...
class Connect2PayClient {
  ...
  protected $fieldsRequired = array('orderID', 'currency', 'amount',
      'shippingType', 'paymentMode');

  protected $fieldsSize = array('afPassword' => 64,
      'currency' => 3, 'ctrlRedirectURL' => 2048,
  ...
  protected $fieldsValidate = array('afClientId' => 'isInt', 'afPassword' => 'isString', 'shopperID' => 'isInt',
      ...
      'ctrlRedirectURL' => 'isAbsoluteUrl', 'ctrlCallbackURL' => 'isAbsoluteUrl', 'timeOut' => 'isString',
      'merchantNotification' => 'isBool', 'merchantNotificationTo' => 'isEmail', 'merchantNotificationLang' => 'isString',
      'themeID' => 'isInt');
  ...
  protected $fieldsJSON = array('afClientId','afPassword', 'apiVersion',
      ...
      'shopperCompany', 'shopperLoyaltyProgram',
      ...
      'ctrlCustomData', 'ctrlRedirectURL', 'ctrlCallbackURL', 'timeOut', 'merchantNotification', 'merchantNotificationTo',
  ...
  /**
   * Validity for the payment link in ISO 8601 duration format. See http://en.wikipedia.org/wiki/ISO_8601.
   * For example: 2 days => P2D, 1 month => P1M
   * @var string
   */
  private $timeOut;
  /**
   * Whether or not to send notification to the merchant after payment processing
   * @var boolean
   */
  ...
  private function doHTTPRequest($type, $url, $data, $assoc = true) {
    ...
    if ($httpCode != 200) {
      $this->clientErrorMessage = "Received HTTP code " . $httpCode . " from connect2pay. ".$curl_error.' '.$curl_errno;
    } else {
      if ($json !== false) {
  ...
  private function initStatus($status) {
      ...
      $reflector = new \ReflectionClass('VPS\Payzone\Api\TransactionStatus');
